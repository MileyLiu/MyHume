        ...
        DispatchQueue.global(qos: .background).async {
           ...
           let imageURL = NSURL(string: imgUrl!)
            ...
            if let _imageURL = imageURL{
                let data = NSData.init(contentsOf: _imageURL as URL)
                                if let _data = data {
                                    let image = UIImage(data: _data as Data)
                                    if let _image = image {
//                                        if let pngData = UIImagePNGRepresentation(_image){
////                                            try! pngData.write(to: URL(string:IMG_PATH)!, options: [.atomicWrite])
//
//                                        }
                                        
                                        do {
                                            try UIImagePNGRepresentation(_image)!.write(to: URL(string:IMG_PATH)!, options: .atomic)
                                        } catch {
                                            print(error)
                                        }
